...
const Projects = () => {
  ...
  const getCategoryStyle = (category: string) => {
    switch (category) {
      case "Frontend":
        return {
          badge:
            "text-cyan-400 border-cyan-500/50 bg-black/80 shadow-[0_0_10px_rgba(34,211,238,0.2)]",
      ...
      case "WordPress":
        return {
          badge:
            "text-blue-400 border-blue-500/50 bg-black/80 shadow-[0_0_10px_rgba(96,165,250,0.2)]",
      ...
      case "UI Design":
        return {
          badge:
            "text-pink-400 border-pink-500/50 bg-black/80 shadow-[0_0_10px_rgba(244,114,182,0.2)]",
      ...
      default:
        return {
          badge:
            "text-purple-400 border-purple-500/50 bg-black/80 shadow-[0_0_10px_rgba(168,85,247,0.2)]",
  ...
  const getBentoClass = (index: number) => {
    ...
    if (patternIndex === 0 || patternIndex === 5)
      return "md:col-span-2 md:row-span-2 lg:col-span-2 lg:row-span-1"; // Wide
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8 border-b border-white/10 pb-8">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-purple-400 font-mono tracking-[0.2em] uppercase text-xs">
                Project Archive
              </span>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            {projectCategories.map((category) => (
              <button
                ...
                className={`px-5 py-2 rounded-lg text-xs font-mono font-bold border transition-all duration-300 ${
                ...
                >
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-end z-20 pointer-events-none">
                    ...
                    <div className="absolute top-6 left-6">
                      <span
                        className={`font-mono text-[10px] uppercase tracking-widest border px-3 py-1.5 rounded-md backdrop-blur-xl font-bold ${styleProps.badge}`}
                      >
                        {project.category}
                      </span>
                    </div>
                    ...
                    <div className="transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                      ...
                      <div className="flex items-center gap-3 mb-3 pointer-events-auto">
                        <a
                          href={`/projects/${project.slug}`}
                          className={`font-bold text-white leading-tight transition-colors ${isWide ? "text-2xl md:text-3xl" : "text-xl"} ${styleProps.textHover}`}
                        >
                          {project.title}
                        </a>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.technologies?.slice(0, 3).map((tech, i) => (
                          <span
                            key={i}
                            className="px-2.5 py-1 rounded text-[10px] bg-black/50 border border-white/10 text-gray-300 font-mono backdrop-blur-sm"
                          >
                            {tech}
                          </span>
                        ))}
        ...
        <div className="mt-20 flex justify-center">
          <a
            ...
            target="_blank"
            className="group flex items-center gap-3 px-8 py-4 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/30 transition-all"
          >
            <span className="text-sm font-mono text-gray-400 group-hover:text-white uppercase tracking-widest">
              View All Projects
            </span>
